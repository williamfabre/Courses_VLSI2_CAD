...
// Formule de rotations vectorielles
...
// pi * 1 << 7 => float PI virgule fixe
short F_PI = (short)((M_PI) * (1<<7));

// tableau des differents arctan pour ne pas les recalculer (approx)
short ATAN[8] = {
	0x64,                 // ATAN(2^-0)
	0x3B,                 // ATAN(2^-1)
	0x1F,                 // ATAN(2^-2)
...
void cordic(short a_p, char x_p, char y_p, char *nx_p, char *ny_p)
{
	...
	// elle est realisee par des simples decalages
	// Soit a entier sur 8 bit
	// Soit na nombre frac avec 7 bit apres la virgule
	// na = a << 7
	// a = na >> 7 (perte de precision)
	a = a_p;
	x = x_p << 7;
	y = y_p << 7;
